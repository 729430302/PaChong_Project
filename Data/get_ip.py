...
class Getip:
    ...
    def findip(self, type, pagenum, targeturl, path):  # ip类型,页码,目标url,存放ip的路径
        ...
        headers = self.getheaders()  # 定制请求头
        html = requests.get(url=url, headers=headers, timeout=10, verify=False).text
        ...
        for i in all:
            ...
            if is_avail == True:
                self.get_txt.write(path=path, text=ip)
                print(ip)

    # -----------------------------------------------------多线程抓取ip入口---------------------------------------------------
    def getip(self, targeturl, path):
        self.get_txt.clear(path)  # 爬取前清空文档
        ...
        ips = self.get_txt.read2(path)  # 读取爬到的ip数量
